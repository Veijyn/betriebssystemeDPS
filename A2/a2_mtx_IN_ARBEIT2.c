...
struct pool {
   ...
   /* *** Dein CODE hier ***/
   // Mutex fuer die Aufgabenqueue
   
   pthread_mutex_t queue;
                 
   // Feld zur Verwaltung der Faeden
...
struct pool* init_pool(){
   ...
   if (pthread_mutex_init(&pool->queue, NULL) != 0){
   ...
   */
	...
	for(i = 0; i < NTHREADS; i++){
		if(pthread_create(&pool->array_threads[i], NULL, &threadpool, pool)!=0){
...
int main(void) {
   ...
   */
   ...
   int i;
   for(i = 0; i < NTHREADS; i++){
		if(pthread_join(pool->array_threads[i],NULL) != 0){
   ...
   /  Gib die Mutex-Variable frei*/
	...
	if (pthread_mutex_destroy(&pool->queue)!=0){
